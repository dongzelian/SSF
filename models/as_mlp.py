...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        if self.tuning_mode == 'ssf':
            x = self.fc1(x) * self.ssf_scale_1.view(1, -1, 1, 1) + self.ssf_shift_1.view(1, -1, 1, 1)
        else:
            x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        if self.tuning_mode == 'ssf':
            x = self.fc2(x) * self.ssf_scale_2.view(1, -1, 1, 1) + self.ssf_shift_2.view(1, -1, 1, 1)
        else:
            x = self.fc2(x) 
...
class AxialShift(nn.Module):
    ...
    def __init__(self, dim, shift_size, as_bias=True, proj_drop=0., tuning_mode=None):
        ...
        if tuning_mode == 'ssf':     
            ...
            self.ssf_scale_3 = nn.Parameter(torch.ones(dim))
            self.ssf_scale_4 = nn.Parameter(torch.ones(dim))
            
            self.ssf_shift_1 = nn.Parameter(torch.zeros(dim))
            self.ssf_shift_2 = nn.Parameter(torch.zeros(dim))
            self.ssf_shift_3 = nn.Parameter(torch.zeros(dim))
            self.ssf_shift_4 = nn.Parameter(torch.zeros(dim))

            # normal init
            nn.init.normal_(self.ssf_scale_1, mean=1, std=.02)
            nn.init.normal_(self.ssf_scale_2, mean=1, std=.02)
            nn.init.normal_(self.ssf_scale_3, mean=1, std=.02)
            nn.init.normal_(self.ssf_scale_4, mean=1, std=.02)

            nn.init.normal_(self.ssf_shift_1, std=.02)
            nn.init.normal_(self.ssf_shift_2, std=.02)
            nn.init.normal_(self.ssf_shift_3, std=.02)
            nn.init.normal_(self.ssf_shift_4, std=.02)
    ...
    def forward(self, x):
        ...
        if self.tuning_mode == 'ssf':   
            x = self.conv1(x) * self.ssf_scale_1.view(1, -1, 1, 1) + self.ssf_shift_1.view(1, -1, 1, 1)
        else:
            x = self.conv1(x)
        ...
        if self.tuning_mode == 'ssf':   
            x_lr = self.conv2_1(x_shift_lr) * self.ssf_scale_2.view(1, -1, 1, 1) + self.ssf_shift_2.view(1, -1, 1, 1)
            x_td = self.conv2_2(x_shift_td) * self.ssf_scale_3.view(1, -1, 1, 1) + self.ssf_shift_3.view(1, -1, 1, 1)
        else:
        ...
        if self.tuning_mode == 'ssf': 
            x = self.conv3(x) * self.ssf_scale_4.view(1, -1, 1, 1) + self.ssf_shift_4.view(1, -1, 1, 1)
        else:
            x = self.conv3(x)
...
class AxialShiftedBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.tuning_mode == 'ssf':
            x = self.norm1(x) * self.ssf_scale_1.view(1, -1, 1, 1) + self.ssf_shift_1.view(1, -1, 1, 1)
        else:
            x = self.norm1(x)
        ...
        if self.tuning_mode == 'ssf':
            x = x + self.drop_path(self.mlp(self.norm2(x) * self.ssf_scale_2.view(1, -1, 1, 1) + self.ssf_shift_2.view(1, -1, 1, 1)))
        else:
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm, tuning_mode=None):
        ...
        if tuning_mode == 'ssf':     
            # add scale and shift
            self.ssf_scale_1 = nn.Parameter(torch.ones(4 * dim)) 
            self.ssf_shift_1 = nn.Parameter(torch.zeros(4 * dim))
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.tuning_mode == 'ssf':  
            x = x * self.ssf_scale_1.view(1, -1, 1, 1) + self.ssf_shift_1.view(1, -1, 1, 1)
            if self.norm is not None:
                x = self.norm(x)
                x = x * self.ssf_scale_2.view(1, -1, 1, 1) + self.ssf_shift_2.view(1, -1, 1, 1)
        else:
            if self.norm is not None:
                x = self.norm(x)
...
class AS_MLP(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        if tuning_mode == 'ssf':   
            # add scale and shift
            self.ssf_scale_1 = nn.Parameter(torch.ones(self.num_features)) 
            self.ssf_shift_1 = nn.Parameter(torch.zeros(self.num_features))
