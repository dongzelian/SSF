...
parser.add_argument('--num-classes-down', type=int, default=None, metavar='N',
                    help='number of label classes (Model default if None)')
...
def main():
    ...
    # create sub_model of nn.Linear named mlp
    # ADD
    model_mlp=nn.Linear(args.num_classes, args.num_classes_down)
    init_params(model_mlp)
    ...
    if mixup_active:
        mixup_args = dict(
            ...
            label_smoothing=args.smoothing, num_classes=args.num_classes_down)
        if args.prefetcher:
...
def train_one_epoch(
    ...
    # check if update gradients of every module in model:
